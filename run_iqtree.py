...
with open(f"{phylip_file}.contree", "r") as file:
    lines = file.readlines()
    lines = [line.strip("\n") for line in lines]
...
with open(f"{phylip_file}.contree", "w+") as file:
...
new_tree_name = os.path.join("niv_evo/trees", os.path.basename(f"{phylip_file}.contree").split(".")[0] + "_iqtree.nwk")
    
subprocess.run(f"mv {phylip_file}.iqtree {new_phylip_name}", shell=True)
subprocess.run(f"mv {phylip_file}.contree {new_tree_name}", shell=True)
