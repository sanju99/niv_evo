...
if len(sys.argv) == 4:
    _, phylip_file, num_bootstrap, out_group = sys.argv
elif len(sys.argv) == 3:
    _, phylip_file, num_bootstrap = sys.argv
    out_group = None
else:
    raise ValueError("3 or 4 command line arguments must be provided!")
...
if out_group is not None:
    subprocess.run(f"iqtree -s {phylip_file} -m MFP -o {out_group} -czb -redo", shell=True)
else:
